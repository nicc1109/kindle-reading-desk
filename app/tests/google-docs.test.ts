...
describe("Google Docs book export", () => {
  ...
  it("does not silently truncate long passages or hundreds of highlights", async () => {
    ...
    expect(bodies[1].requests[1].updateTextStyle.textStyle.bold).toBe(false);
    expect(bodies.slice(2).flatMap((body) => body.requests)).toHaveLength(405);
    expect(bodies[2].requests).toHaveLength(200);
  });
